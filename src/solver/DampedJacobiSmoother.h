...
namespace Meso {
	...
	public:
		...
		DampedJacobiSmoother(MaskedPoissonMapping<T, d>& _poisson, const ArrayDv<T>& _one_over_diag, const int _iter_num, 
			const int _boundary_iter_num, const real _omega = 2.0 / 3.0) { Init(_poisson, _one_over_diag, _iter_num, _boundary_iter_num, _omega); }
		void Init(MaskedPoissonMapping<T, d>& _poisson, const ArrayDv<T>& _one_over_diag, const int _iter_num, 

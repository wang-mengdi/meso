...
namespace Meso {
	...
	void Restrictor_Axis_Kernel(const int axis, const Grid<d> grid, T* intp_data, const T* original_data) {
		Typedef_VectorD(d);
		VectorDi coord = GPUFunc::Thread_Coord<d>(blockIdx, threadIdx);
		T data0, data1, data2, data3;
		data1 = original_data[grid.Index(coord)];
		coord[axis]--;
		if (coord[axis] < 0) data0 = data1;
		else data0 = original_data[grid.Index(coord)];
		coord[axis] += 2;
		if (coord[axis] >= grid.counts[axis]) data2 = data1;
		else data2 = original_data[grid.Index(coord)];
		coord[axis]++;
		if (coord[axis] >= grid.counts[axis]) data3 = data2;
		else data3 = original_data[grid.Index(coord)];
		intp_data[idx1] = (data0 + 3 * data1 + 3 * data2 + data3) / 8;
	}

	template<class T, int d>
	void Restrictor_Coarser_Kernel(const int axis, const Grid<d> coarser_grid, T* coarser_data, const Grid<d> finer_grid, const T* finer_data) {
		Typedef_VectorD(d);
	...
	class Restrictor : public LinearMapping<T> {
		...
		virtual void Apply(ArrayDv<T>& Ap, const ArrayDv<T>& p) {
			T* intp_ptr = ArrayFunc::Data(intp_data);
			const T* original_ptr = ArrayFunc::Data(p);
			for (int axis = 0; axis < d; axis++) {
				finer_grid.Exec_Kernel(&Restrictor_Axis_Kernel<T, d>, axis, intp_ptr, original_ptr);
			}
			coarser_grid.Exec_Kernel(&Restrictor_Coarser_Kernel<T, d>, coarser_grid, ArrayFunc::Data(Ap), finer_grid, intp_ptr);
		}
	};
}
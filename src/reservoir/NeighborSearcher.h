...
//			AuxFunc::Assert(0 <= idx && idx < search_results.size(), "NeighborSearcher error: index overflow");
...
//		}
//		virtual std::shared_ptr<Array<VectorD> > Points_Ptr(void) { return points_ptr; }
...
//				results.reserve(results.size() + num);

...
namespace Meso {
	...
	real Initialize_Box_Rim_Points_2D(const Vector2& origin, const Vector2i& pad_size, const Vector2i& int_size, const Vector2& _dx, Points& pts, Array<Vector2>& pos, bool keep_existing = false, std::function<void(const int idx)> instruction = nullptr);

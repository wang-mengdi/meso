...
template<class T_KEY,class T> bool Has(Hashtable<T_KEY,T>& hashtable,const T_KEY& key)
{auto iter=hashtable.find(key);return iter!=hashtable.end();}

template<class T_KEY,class T> bool Has(Hashtable<T_KEY,T>& hashtable,const T_KEY& key,const T& v)
{auto iter=hashtable.find(key);return iter.second==v;}
...
template<class T_KEY,class T> bool Add(Hashtable<T_KEY,T>& hashtable,const T_KEY& key,const T& v,bool check_duplicate=false)
...
template<class T_KEY,class T> bool Remove(Hashtable<T_KEY,T>& hashtable,const T_KEY& key,const T& v)
{auto iter=hashtable.find(key);if(iter!=hashtable.end()){hashtable.erase(iter);return true;}return false;}
...
inline Vector<int,1> Unique_Ordered(const Vector<int,1>& e){return e;}

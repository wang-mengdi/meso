...
namespace Meso {
	...
	public:
		...
		void Print_Frame_Info(Timer &frame_timer, const DriverMetaData& meta_data) {
			...
			Info("Frame {} in {}-{} done in {:.3f}s, total time cost {:.3f}s, ETA {:.3f}/{:.3f}s", meta_data.current_frame, meta_data.first_frame, meta_data.last_frame, frame_seconds, completed_seconds, eta, completed_seconds + eta);
		}
		//will change timer
		...
		void Advance(Simulator &simulator, DriverMetaData& meta_data) {
			...
			if (meta_data.output_each_step) {
				while (meta_data.current_time<meta_data.total_time) {
					meta_data.dt = simulator.CFL_Time(meta_data.cfl);
					if (meta_data.current_time + meta_data.dt > meta_data.total_time) { 
						meta_data.dt = meta_data.total_time - meta_data.current_time;
						meta_data.current_time = meta_data.total_time; 
					}
					else{ meta_data.current_time += meta_data.dt; }
					simulator.Advance(meta_data);

					Print_Frame_Info(frame_timer, meta_data);
					Info("Output time {} to {}", meta_data.current_time, meta_data.output_base_dir);
					simulator.Output(meta_data);
					meta_data.current_frame += 1;
				}
			}

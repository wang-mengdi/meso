...
namespace Meso {
	template<int d>
	class  FluidPICImpulseInitializer {
	public:
		Typedef_VectorD(d);
		...
		void Apply(json& j, FluidPICImpulse<d>& fluid) {
		...
		void Case_0(json& j, FluidPICImpulse<d>& fluid) {
			...
			int reinit_threshold = Json::Value(j, "reinit_threshold", 10);
			fluid.Init(fixed, vol, face_fixed, initial_vel, reinit_threshold);
			ArrayFunc::Fill(fluid.velocity_host.Data(0), 1.0);
		}

		void Case_1(json& j, FluidPICImpulse<d>& fluid) {
			int scale = Json::Value(j, "scale", 32);
			real dx = 1.0 / scale;
		...
		void Case_2(json& j, FluidPICImpulse<d>& fluid) {
			int scale = Json::Value(j, "scale", 32);
			real dx = 1.0 / scale;
			VectorDi grid_size = scale * MathFunc::Vi<d>(2, 1, 1);
			Grid<d> grid(grid_size, dx, VectorD::Zero(), CENTER);

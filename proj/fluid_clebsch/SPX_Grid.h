...
{	Typedef_VectorDii(d);
public:
	VectorDi cell_counts=VectorDi::Zero();
	VectorDi node_counts=VectorDi::Zero();
	real dx=(real)0;
	VectorD domain_min=VectorD::Zero();
	VectorD domain_max=VectorD::Zero();

	////Initializer
	Grid();
	Grid(const VectorDi& _cell_counts,const real _dx=(real)0,const VectorD& _domain_min=VectorD::Zero());
	...
	static int Nb_C_Side(const int i);													////return C neighbor side by index
...
#pragma omp parallel for
	...
	decltype(auto) Sum_All(Fcell f) const {
		using T = decltype(f(VectorDi::Zero()));
		const int cell_num = Number_Of_Cells();
		T sum = 0;

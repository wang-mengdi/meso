...
template<int d>
void CodimMesher<d>::Reset()
{
	invalid_particle_hashset.clear();
	invalid_particle_heap = Heap<int, std::greater<int> >();
	invalid_element_hashset.clear();
	invalid_element_heap = Heap<int, std::greater<int> >();
}
...
{
	int op_num=0;int pre_op_num=0;int min_op_num=3;int max_iter_num=10;
				...
				if(use_triangle_edge_collapse){
					...
					if(edge_to_collapse!=Vector2i(-1,-1)){
						if(Collapse_Triangle_Edge(edge_to_collapse)){op_num++;
							if(dbg)Dbg_Check("edge collapse");
							continue;}}}
...
template<int d> void CodimMesher<d>::Collapse_Segment_Edge(const int i)
{
	...
	Remove_Element(i);
	for (const auto& i : a_incident_elements)
	{
		Replace_Vertex<4>(E(i), a, v);
		Add_Buffer_Element(i);
	}
...
template<int d> bool CodimMesher<d>::Remove_Triangle(const int e)
{
	...
	for(int i=0;i<3;i++)if(Value_Count(aux.vertex_element_hashtable,vtx[i])==0)Remove_Particle(vtx[i]);
	return true;
}
...
template<int d> bool CodimMesher<d>::Remove_Segment(const int e)
{
	...
	for(int i=0;i<2;i++)if(Value_Count(aux.vertex_element_hashtable,vtx[i])==0)Remove_Particle(vtx[i]);
	return true;
}
...
template<int d> void CodimMesher<d>::Pruned_Copy(CodimMesh<d>& pruned_mesh,CodimParticles<d>& pruned_particles,std::shared_ptr<Array<int> >* _remap)
{
	...
	int j=0;for(int i=0;i<particles.Size();i++)
		if(Valid_Particle(i)){pruned_particles.Copy_Element_From(j,particles,i);(*remap)[i]=j;j++;}

	for(auto& e:pruned_mesh.elements){const int ds=CodimMesh<d>::Dim(e);
		for(int i=0;i<ds;i++)e[i]=(*remap)[e[i]];
		for(int i=ds;i<4;i++)e[i]=-1;}

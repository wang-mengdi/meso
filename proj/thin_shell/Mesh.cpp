...
template<int d,int e_d> void SimplicialMesh<d,e_d>::Write_Binary(std::ostream&output) const
{
	...
	if (vtx_n > 0) File::Write_Binary_Array(output, &(*vertices)[0], vtx_n);
...
Inst_Helper(SegmentMesh<3>, SimplicialMesh3);
Inst_Helper(SimplicialMesh3,SegmentMesh<3>);
Inst_Helper(SegmentMesh<2>, SimplicialMesh2);
Inst_Helper(SimplicialMesh2, SegmentMesh<2>);
#undef Inst_Helper

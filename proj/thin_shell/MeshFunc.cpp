...
namespace MeshFunc{
    ...
    real Tetrahedron_Volume(const Vector3& v0,const Vector3& v1,const Vector3& v2,const Vector3& v3){return CommonConstants::one_sixth*abs((v1-v0).cross(v2-v0).dot(v3-v0));}
    ...
    template<> Vector<real,3> Element_Center<3>(const Array<Vector3>& v){return CommonConstants::one_third*(v[0]+v[1]+v[2]);}
	...
	template<class T_MESH> void Merge(const Array<T_MESH*>& meshes, T_MESH* merged_mesh)
	{
		...
		int n=0;for(auto& m:meshes){
			...
			for(auto& e:m->elements){
				auto e1=e;for(int i=0;i<e_d;i++)e1[i]+=n;

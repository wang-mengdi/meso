...
public:
	...
	virtual int Add_Elements(const int n) { array_ptr->resize(array_ptr->size() + n); return (int)array_ptr->size() - n; }
	...
	virtual int Read_Binary(const std::string& file_name) { return BinaryDataIO::Read_Array<T>(file_name, *array_ptr); }
	virtual int Write_Binary(const std::string& file_name) { return BinaryDataIO::Write_Array<T>(file_name, *array_ptr); }
	virtual void Read_Binary_Content(std::istream& input, const uint32_t& n) { BinaryDataIO::Read_Array_Stream_Content<T>(input, *array_ptr, n); }
	virtual void Write_Binary_Content(std::ostream& output) { BinaryDataIO::Write_Array_Stream_Content<T>(output, *array_ptr); }

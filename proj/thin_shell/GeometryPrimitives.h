...
//
//	virtual bool Inside(const VectorD& pos) const {return VectorFunc::All_Greater_Equal(pos,min_corner)&& VectorFunc::All_Less_Equal(pos,max_corner);}
//	virtual real Phi(const VectorD& pos) const 
//	{VectorD phi=(pos-Center()).cwiseAbs()-(real).5*Edge_Lengths();
//	if (phi.maxCoeff()>0) { return 0; }return phi.maxCoeff();} //changed
...
//{Typedef_VectorD(d);using Base=Box<d>;
//public:
...
//		return VectorFunc::Cross(p0q,p01)[0]<0;
//	}
//
...
//{Typedef_VectorD(d);					// normal not in use, perhaps someone knows SDF better can add support to normal
//public:
//	VectorD center = VectorD::Zero();
//	real radius = (real).1;

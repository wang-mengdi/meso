...
namespace Meso {
	...
	public:
		...
		bool Cell_In_Boundary(const Grid<d> grid, const VectorDi cell, int axis, int side, int width) {
			...
			else if (side == 1) return cell[axis] >= grid.cell_counts[axis] - width;
			return false;
		}
		...
		bool Face_In_Boundary(const Grid<d> grid, int axis, const VectorDi face, int chk_axis, int side, int width) {
			if (side == 0) {
				...
				return face[chk_axis] >= grid.cell_counts[chk_axis] - width;
			}
			else return false;
		}
			...
			grid.Exec_Faces(
				[&](const int axis, const VectorDi face) {
					...
					for (int chk_axis = 0; chk_axis < d; chk_axis++) {
						for (int side = 0; side < 2; side++) {
							if (Face_In_Boundary(axis, face, chk_axis, side, bc_width(chk_axis, side))) {
								in_cnt++;
								_chk_axis = axis;
								_side = side;
							}

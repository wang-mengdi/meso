			...
			[&](const VectorDi cell) {
				...
				if (fixed(cell)) {
					...
					for (int axis = 0; axis < d; axis++) {
						if(idx == 0) {
							Info("cell {} nb face {}, {} vol {}", cell, axis, cell, vol(axis, cell));
							Info("cell {} nb face {}, {} vol {}", cell, axis, cell + VectorDi::Unit(axis), vol(axis, cell + VectorDi::Unit(axis)));
						}
